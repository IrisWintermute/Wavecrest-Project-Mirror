...
# shared vars
export scripts_location="$(realpath ../../../tf-connect-${part}/scripts)"

## set up kafka provisioner
export jump_host="admin@${envname}-${short_region}-${colour}-jump"
log "Setting up kafka provisioner on jump host ${jump_host}"

command_ssh 'sudo apt-get update && DEBIAN_FRONTEND=noninteractive sudo apt-get install -qq --assume-yes rsync'

log "RSYNC: rsync -av -e 'ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR' ${scripts_location}/ ${jump_host}:/tmp/kafka"
rsync -av -e 'ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR' ${scripts_location}/ ${jump_host}:/tmp/kafka

log "Running provisioner on jump host ${jump_host}"

command_ssh "cd /tmp/kafka && ./create_topics.sh > /tmp/kafka_provision.log 2>&1"

if [[ "${debug}" == "true" ]]; then
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${jump_host} -t "cat /tmp/kafka_provision.log"
fi
